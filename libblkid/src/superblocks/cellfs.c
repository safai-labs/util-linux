/*
 * Copyright (C) 2021 Ahmed Masud <ahmed.masud@saf.ai>
...
#define CELLFS_SB_MAGIC		0x8888
#define CELLFS_SB_OFFSET		0x400
#define CELLFS_SBB_OFFSET(_sz)	((((_sz) / 0x200) - 8) * 0x200)

static int cellfs_valid_sb(blkid_probe pr, struct cellfs_super_block *sb, int is_bak)
{
	static unsigned char sum[4];
	const int sumoff = offsetof(struct cellfs_super_block, s_sum);
	size_t bytes;
	const size_t crc_start = sumoff + 4;
	uint32_t crc;

	if (!sb || le16_to_cpu(sb->s_magic) != CELLFS_SB_MAGIC)
		return 0;
	 ...
	 * sb */
	if (bytes < crc_start || bytes > sizeof(struct cellfs_super_block))
		return 0;
...
static int probe_cellfs(blkid_probe pr,
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	struct cellfs_super_block *sb, *sbp, *sbb;
	int valid[2], swp = 0;
	uint64_t magoff;

	/* primary */
	sbp = (struct cellfs_super_block *) blkid_probe_get_buffer(
			pr, CELLFS_SB_OFFSET, sizeof(struct cellfs_super_block));
	if (!sbp)
		return errno ? -errno : 1;

	valid[0] = cellfs_valid_sb(pr, sbp, 0);


	/* backup */
	sbb = (struct cellfs_super_block *) blkid_probe_get_buffer(
			pr, CELLFS_SBB_OFFSET(pr->size), sizeof(struct cellfs_super_block));
	if (!sbb) {
		valid[1] = 0;
		...
		valid[1] = cellfs_valid_sb(pr, sbb, 1);
	...
	DBG(LOWPROBE, ul_debug("cellfs: primary=%d, backup=%d, swap=%d",
				valid[0], valid[1], swp));
	...
	magoff = swp ? CELLFS_SBB_OFFSET(pr->size) : CELLFS_SB_OFFSET;
	magoff += offsetof(struct cellfs_super_block, s_magic);
...
const struct blkid_idinfo cellfs_idinfo =
{
	.name		= "cellfs",
	.usage		= BLKID_USAGE_FILESYSTEM,
	.probefunc	= probe_cellfs,
	/* default min.size is 128MiB, but 1MiB for "mkfs.cellfs -b 1024 -B 16" */
	.minsz		= (1024 * 1024),
